...
(opt into the moment-shape value, §5.5b).
...
open section uses the `C_mLT` form. **`C_m = 1.0`** by default (uniform equivalent moment, the Table B.3
upper bound), so the factors are conservative for any real moment shape; `--moment-shape` (§5.5b) derives
`C_my`/`C_mz` from the moment diagram instead. `χ_LT` enters exactly as in the code
...
### 5.5b Moment-shape factors `C₁` and `C_m` (`--moment-shape`, opt-in, default off)
By default the LTB factor `C₁` (§5.4) and the 6.3.3 equivalent-uniform-moment factors `C_m` (§5.5) are
held at **1.0** — the uniform-moment value, conservative for *any* real diagram. With `--moment-shape`
they are derived from the actual moment shape, dropping that conservatism where the diagram is known:

- **`C₁`** uses the general **4-moment / `C_b` formula**
  `C₁ = 12.5·M_max / (2.5·M_max + 3·M_¼ + 4·M_½ + 3·M_¾)` on moment magnitudes — `1.0` for a uniform
  moment, **`1.136`** for a simply-supported beam under uniform load, capped at `2.70` and floored at
  `1.0`. A higher `C₁` raises `M_cr` (linearly) and hence `χ_LT`, so an LTB-governed beam is checked
  less conservatively.
- **`C_m`** uses **Annex B Table B.3** `C_m = 0.6 + 0.4·ψ ≥ 0.4` from the signed end-moment ratio `ψ`,
  but **only when the member is end-moment driven** (the peak sits at an end — a column under a lateral
  case); a transverse-loaded beam (peak interior, ≈0 end moments, and ≈0 axial anyway) keeps the
  conservative `C_m = 1.0`.

Where the shape comes from: the **analytic** path knows every beam is a simply-supported UDL span
(`C₁ = 1.136`); the **frame** path samples the *solved* PyNite moment at `x = 0, L/4, L/2, 3L/4, L`
(any read failure falls back to `1.0`); the unrestrained **construction-stage** and **wind-uplift**
entries — the cases where LTB actually governs — also take `C₁ = 1.136`. Because `C₁ > 1` and
`C_m < 1` are *less* conservative, they are only ever taken from a known or solved diagram, are capped
and floored, and **default off** so existing results are byte-identical. On the real case-study
building the headline is unchanged (its beams are slab-restrained, so LTB does not govern, and the
match is length-limited); under `--construction` the booked CO₂ dips a little because the sharper check
also lightens the *avoided-new baseline* (the lightest new section that now passes the erection stage),
which is the honest direction.

**Effective length and `k`.** Flexural-buckling and LTB use the system length with `k = 1.0` by
default. This is justified by the **non-sway classification** (§4.1): `α_cr ≥ 10` per EN 1993-1-1
5.2.1(4)B means the `k = 1.0` route is sound, and a sway-sensitive frame (`α_cr < 10`) is warned and
carried by the **second-order (P-Δ) solve**, under which `k = 1.0` remains valid (the amplification is
captured in the analysis, not in an effective-length multiplier). A per-member override `k_y`/`k_z`
(JSON/CSV) is honoured for members whose real end conditions are known to differ.
...
5. **Cutting-stock (the default; `--no-cut` restricts to whole-member reuse).** Reclamation stockists
   cut members to length as a matter of course, so by default a donor may
...
**Whole suite:** 522 tests, ruff clean.
